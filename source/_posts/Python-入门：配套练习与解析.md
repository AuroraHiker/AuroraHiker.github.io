...
- 数据分析中，初始化值、数据清洗方式、异常处理策略等都应当根据具体问题具体分析，避免套用固定模板。例如，同样是求最大值，价格数据从0开始合理，但温度数据可能从负数开始，适用场景不同。
...
- **忽视输入为空**：直接对空字符串执行 split() 会得到空列表，后续循环不会执行，但 count 为 0 时最终会提示“无有效成绩”。不过最好显式检查输入是否为空，提升用户体验。
...
- **忽视空数据**：若过滤后有效列表为空，len(valid_datas) 为 0，total / len(valid_datas) 会触发 ZeroDivisionError，务必提前检查。
- **边界判断**：高于平均值使用 >，若需求是“不低于平均值”则应使用 >=。根据业务要求选择正确的比较符号。
...
- **直接比较字符串：**初学者容易写成 `"09:05" > "08:30"`，认为这种写法等同于时间比较。字符串比较是按字典序逐字符进行的，虽然在这个特定例子中碰巧有效，但遇到跨小时（如 `"08:59"` 与 `"09:00"`）或跨天的情况时，字符串比较会完全失效。必须转换为同一单位（如分钟数）再进行数值比较。
...
作为对比，这里展示选择排序的实现，帮助读者理解不同算法的差异：
...
1. 处理空列表：若列表为空，直接返回 `None`，避免除零错误。过滤非法数据。
2. 初始化统计变量：
   - `total` 用于累加总分
